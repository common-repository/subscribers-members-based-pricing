...
class AppliedProducts {
	...
	public function getName() {
		return __( 'Products', 'subscribers-members-based-pricing' );
	}
	...
	public function render( GlobalPricingRule $rule ) {
		...
		if ( ! empty( $appliedCategories ) ) {
			...
			esc_html_e( 'Categories: ', 'subscribers-members-based-pricing' );
		...
		if ( ! empty( $appliedProducts ) ) {
			...
			esc_html_e( 'Products: ', 'subscribers-members-based-pricing' );
		...
		if ( empty( $appliedProducts ) && empty( $appliedCategories ) ) {
			?>
			<b style="color:#d63638"><?php esc_html_e( 'Applied to every product', 'subscribers-members-based-pricing' ); ?></b>
			<?php
		}
	}
}
